...
- `improved` :
  - `true` si mode solo et `finalScore > previousBest` (ou première partie solo de ce joueur)
  - `false` si mode solo et `finalScore <= previousBest`
  - `null` en mode 1v1 (chaque match est unique, la notion de record perso ne s'applique pas)
- `previousBest` : le meilleur score solo précédent (`null` si première partie ou en 1v1)
- ⚠️ La Game est **toujours persistée** (même si `improved=false`) — on garde l'historique complet. Le filtre se fait à la lecture (`MAX(score) GROUP BY` côté leaderboard, flag `is_best` côté historique).
...
| `GET /players/{id}/games?mode=&limit=` | Historique des games terminées du joueur, DESC sur `finished_at`. `mode` optionnel, `limit ∈ [1, 100]` (défaut `20`). 🆕 Chaque game porte un flag `is_best: bool` (true uniquement pour la game solo ayant le meilleur score du joueur). | `200` (liste vide si pas de games) ou `404` (joueur inconnu) |
